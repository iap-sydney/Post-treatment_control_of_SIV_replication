...
data0 <-read_excel("Input SP_vs_DNA.xlsx", sheet=1)
...
m1a=lme(sp~1+dna, random = list(cohort = pdDiag(~ dna)), data=data1,control=ctrl,method="ML")
summary(m1a)
anova(m1a)
qqnorm(resid(m1a))
...
print("Model withot random group effect"  )
ctrl <- lmeControl(opt='optim');
m1b=lme(sp~1+dna, random =~ 1|cohort, data=data1,control=ctrl,method="ML")
summary(m1b)
anova(m1b)
qqnorm(resid(m1b))
